...
class SQLiteDatabase:
    ...
    def fetchall(self, tablename, fetch_fields='*', condition=None, distinct=False):
        """
        ...
        if condition is None:
            self._cursor.execute("""SELECT {dist}{field} FROM {table}""".format(
                field=','.join(fetch_fields), table=tablename, dist=distinct_str
            ))
        elif type(condition) == dict:
            if len(condition) == 1:
                key = list(condition.keys())[0]
                self._cursor.execute("""SELECT {dist}{field} FROM {table} WHERE {id_field}={id}""".format(
                field=','.join(fetch_fields), table=tablename, id_field=key, id=condition[key], dist=distinct_str
            ))
            else:
                insertion = ' AND '.join(
                    ["{k}={v}".format(k=key, v=condition[key]) for key in condition]
                )
                self._cursor.execute("""SELECT {dist}{field} FROM {table} WHERE {cond}""".format(
                    field=','.join(fetch_fields), table=tablename, cond=insertion, dist=distinct_str
                ))
        elif type(condition) == str:
            self._cursor.execute("""SELECT {dist}{field} FROM {table} WHERE {cond}""".format(
                    field=','.join(fetch_fields), table=tablename, cond=condition, dist=distinct_str
                ))
