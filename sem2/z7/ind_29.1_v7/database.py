...
class SQLiteDatabase:
    ...
    def find(self, tablename, chat_id, field='*'):
        self.__connect()
        self._cursor.execute("""SELECT {fields} FROM {table} WHERE chat_id={id}""".format(
            fields=field, table=tablename, id=chat_id
        ))
    ...
    def show_table(self, tablename):
        self.__connect()
        self._cursor.execute("""SELECT * FROM {}""".format(tablename))
        res = self._cursor.fetchall()
        self._connection.close()
        return res
